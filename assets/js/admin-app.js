...
( function () {
	...
	function AdminApp() {
		...
		// Load analytics data
		var loadAnalytics = useCallback( function ( days ) {
			var d = days || 30;
			return api( 'analytics?days=' + d ).then( function ( data ) {
		...
		var loadConfig = useCallback( function () {
			...
			Promise.all( [
				...
				setSynonyms( res[1] && res[1].synonyms ? res[1].synonyms : [] );
		...
		return createElement( 'div', null,
			...
			currentPage === 'wpts-settings' && createElement( 'div', { className: 'wpts-settings-layout' },
				...
				createElement( 'div', { className: 'wpts-settings-content' },
					...
					( tab === 'engines'   && createElement( EnginesTab, { settings: settings, update: updateSetting, showToast: showToast, save: saveSettings, saving: saving, isDirty: isDirty } ) ) ||
			...
			currentPage === 'wpts-cache' && createElement( 'div', { className: 'wpts-settings-layout' },
				...
				createElement( 'div', { className: 'wpts-settings-content' },
					( tab === 'general'   && createElement( CacheDriversTab, { settings: settings, update: updateSetting, showToast: showToast } ) ) ||
					( tab === 'redis'     && createElement( RedisConfigTab, { settings: settings, update: updateSetting, showToast: showToast } ) ) ||
					( tab === 'memcached' && createElement( MemcachedConfigTab, { settings: settings, update: updateSetting, showToast: showToast } ) ) ||
					( tab === 'cdn'       && createElement( CdnHeadersTab, { settings: settings, update: updateSetting } ) ) ||
					( tab === 'flush'     && createElement( FlushCacheTab, { showToast: showToast, loadConfig: loadConfig } ) ) ||
					createElement( CacheDriversTab, { settings: settings, update: updateSetting, showToast: showToast } )
				)
			),
	...
	function SearchLogTab( props ) {
		var initialLog = props.analytics && props.analytics.search_log;
		var initialItems = Array.isArray( initialLog ) ? initialLog : ( ( initialLog && initialLog.items ) || [] );
		var initialTotal = ( initialLog && initialLog.total ) !== undefined ? initialLog.total : initialItems.length;
		var initialPages = ( initialLog && initialLog.total_pages ) !== undefined ? initialLog.total_pages : ( initialTotal > 0 ? 1 : 0 );
			...
			createElement( 'div', { className: 'wpts-log-toolbar' },
				...
				createElement( 'div', { style: { display: 'flex', alignItems: 'center', gap: 8, marginLeft: 'auto' } },
					...
					createElement( 'select', {
						...
						style: { width: 75, padding: '4px 8px', fontSize: 12, height: 32 }
					},
								...
								createElement( 'td', null,
									topKeywords.length ? createElement( 'div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6 } },
										topKeywords.map( function ( kw, kwIdx ) {
											var searchUrl = homeUrl + '?s=' + encodeURIComponent( kw.query );
											return createElement( 'a', {
												key: kwIdx,
												href: searchUrl,
												target: '_blank',
												...
												title: 'Click to test search "' + kw.query + '" on your site (' + kw.keyword_clicks + ' clicks at Rank #' + r.position + ')'
											},
				...
				keywordsBreakdown.length ? createElement( 'table', { className: 'widefat striped', style: { border: '1px solid #e2e8f0', marginTop: 12 } },
					...
					createElement( 'tbody', null,
						keywordsBreakdown.map( function ( row, idx ) {
							var searchUrl = homeUrl + '?s=' + encodeURIComponent( row.query );
							return createElement( 'tr', { key: idx },
								createElement( 'td', null,
									createElement( 'a', {
										...
										target: '_blank',
										rel: 'noopener noreferrer',
										style: { fontWeight: 700, color: '#2563eb', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 5 }
									},
										row.query,
	...
	function ExportsTab( props ) {
		...
		return createElement( 'div', { className: 'wpts-app-panel' },
			createElement( 'h3', { className: 'wpts-app-panel__title' }, '⬇️ Export Search Analytics to CSV' ),
			createElement( 'div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16, marginTop: 16 } },
				createElement( 'div', { style: { border: '1px solid #e2e8f0', padding: 16, borderRadius: 8 } },
					...
					createElement( 'a', { href: url + '?action=wpts_export_searches&mode=searches&_wpnonce=' + nonce, className: 'button button-primary' }, '⬇ Export Searches (CSV)' )
				),
				createElement( 'div', { style: { border: '1px solid #e2e8f0', padding: 16, borderRadius: 8 } },
					...
					createElement( 'a', { href: url + '?action=wpts_export_searches&mode=top&_wpnonce=' + nonce, className: 'button' }, '⬇ Export Top Queries (CSV)' )
				),
				createElement( 'div', { style: { border: '1px solid #e2e8f0', padding: 16, borderRadius: 8 } },
					...
					createElement( 'a', { href: url + '?action=wpts_export_searches&mode=zero&_wpnonce=' + nonce, className: 'button' }, '⬇ Export Gaps (CSV)' )
				)
			),
		...
		return createElement( 'div', { className: 'wpts-app-panel' },
			...
			createElement( 'div', { className: 'wpts-form-group' },
				...
				createElement( 'select', {
					...
					onChange: function ( e ) { u( 'cache_driver', e.target.value ); }
				},
	...
	function CdnHeadersTab( props ) {
		...
		return createElement( 'div', { className: 'wpts-app-panel' },
			...
			createElement( 'div', { className: 'wpts-form-group', style: { marginTop: 16 } },
				createElement( 'label', { className: 'wpts-form-label' }, 'CDN Edge Max-Age (seconds)' ),
				createElement( 'input', { type: 'number', className: 'wpts-form-input', value: s.cdn_max_age || 300, onChange: function ( e ) { u( 'cdn_max_age', parseInt( e.target.value, 10 ) ); } } )
			)
		);
	}
		...
		return createElement( 'div', { className: 'wpts-app-panel' },
			...
			createElement( 'div', { className: 'wpts-form-group' },
				...
				createElement( 'select', {
					...
					onChange: function ( e ) { u( 'search_engine', e.target.value ); }
				},
			...
			s.search_engine === 'typesense' && createElement( 'div', { style: { marginTop: 20, padding: 16, background: '#f8fafc', borderRadius: 8, border: '1px solid #e2e8f0' } },
				...
				createElement( 'div', { className: 'wpts-form-group' },
					...
					createElement( 'input', { className: 'wpts-form-input', value: s.typesense_host || '', onChange: function ( e ) { u( 'typesense_host', e.target.value ); } } )
				),
				createElement( 'div', { className: 'wpts-form-group' },
					createElement( 'label', { className: 'wpts-form-label' }, 'Port' ),
					createElement( 'input', { className: 'wpts-form-input', value: s.typesense_port || '8108', onChange: function ( e ) { u( 'typesense_port', e.target.value ); } } )
				),
				createElement( 'div', { className: 'wpts-form-group' },
					createElement( 'label', { className: 'wpts-form-label' }, 'Protocol' ),
					createElement( 'select', { className: 'wpts-form-select', value: s.typesense_protocol || 'http', onChange: function ( e ) { u( 'typesense_protocol', e.target.value ); } },
				...
				createElement( 'div', { className: 'wpts-form-group' },
					...
					createElement( 'input', { type: 'password', className: 'wpts-form-input', value: s.typesense_api_key || '', onChange: function ( e ) { u( 'typesense_api_key', e.target.value ); } } )
				),
				createElement( 'button', { type: 'button', className: 'button', disabled: testing, onClick: function () { testEngine( 'typesense' ); } }, '🔌 Test Typesense' ),
				tsStatus && createElement( 'span', { style: { marginLeft: 10, fontSize: 13 } }, tsStatus )
			),
			...
			s.search_engine === 'elasticsearch' && createElement( 'div', { style: { marginTop: 20, padding: 16, background: '#f8fafc', borderRadius: 8, border: '1px solid #e2e8f0' } },
				...
				createElement( 'div', { className: 'wpts-form-group' },
					createElement( 'label', { className: 'wpts-form-label' }, 'Host' ),
					createElement( 'input', { className: 'wpts-form-input', value: s.elasticsearch_host || '', onChange: function ( e ) { u( 'elasticsearch_host', e.target.value ); } } )
				),
				createElement( 'div', { className: 'wpts-form-group' },
					createElement( 'label', { className: 'wpts-form-label' }, 'Port' ),
					createElement( 'input', { className: 'wpts-form-input', value: s.elasticsearch_port || '9200', onChange: function ( e ) { u( 'elasticsearch_port', e.target.value ); } } )
				),
				createElement( 'div', { className: 'wpts-form-group' },
					createElement( 'label', { className: 'wpts-form-label' }, 'Protocol' ),
					createElement( 'select', { className: 'wpts-form-select', value: s.elasticsearch_protocol || 'http', onChange: function ( e ) { u( 'elasticsearch_protocol', e.target.value ); } },
						createElement( 'option', { value: 'http' }, 'HTTP' ),
						createElement( 'option', { value: 'https' }, 'HTTPS' )
					)
				),
				createElement( 'div', { className: 'wpts-form-group' },
					createElement( 'label', { className: 'wpts-form-label' }, 'API Key or Password' ),
					createElement( 'input', { type: 'password', className: 'wpts-form-input', value: s.elasticsearch_api_key || '', onChange: function ( e ) { u( 'elasticsearch_api_key', e.target.value ); } } )
				),
				createElement( 'button', { type: 'button', className: 'button', disabled: testing, onClick: function () { testEngine( 'elasticsearch' ); } }, '🔌 Test Elasticsearch' ),
				esStatus && createElement( 'span', { style: { marginLeft: 10, fontSize: 13 } }, esStatus )
			),
			...
			s.enable_vector_search && createElement( 'div', { style: { marginTop: 20, padding: 16, background: '#f8fafc', borderRadius: 8, border: '1px solid #e2e8f0' } },
				createElement( 'div', { className: 'wpts-form-group' },
					...
					createElement( 'select', {
						...
						onChange: function ( e ) { u( 'vector_provider', e.target.value ); }
					},
	...
	function IndexerTab( props ) {
		...
		var indexSinglePost = function () {
			...
			api( 'reindex-post', 'POST', { post_id: idNum } )
				...
				.catch( function ( err ) {
					setSingleLoading( false );
					alert( ( err && err.message ) || 'Error indexing post.' );
				} );
		};
		...
		return createElement( 'div', { className: 'wpts-app-panel' },
			...
			createElement( 'div', { style: { maxWidth: 500, marginBottom: 20 } },
				...
				createElement( 'small', { style: { color: '#64748b' } }, ( ( c.indexed_count || 0 ) ).toLocaleString() + ' of ' + ( ( c.total_posts || 0 ) ).toLocaleString() + ' posts currently indexed.' )
			),
			...
			createElement( 'div', { style: { marginTop: 32, paddingTop: 24, borderTop: '1px solid #e2e8f0' } },
				...
				singleResult && createElement( 'div', {
					...
					createElement( 'div', { style: { display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700, color: '#166534', marginBottom: 4 } },
						'✅ Item #' + singleResult.post_id + ' ("' + singleResult.post_title + '") Indexed!'
					),
					createElement( 'div', { style: { fontSize: 12.5, color: '#15803d', display: 'flex', gap: 16, flexWrap: 'wrap' } },
