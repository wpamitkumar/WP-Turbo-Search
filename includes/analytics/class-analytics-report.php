...
class AnalyticsReport {
	...
	public static function get_ctr_by_position( int $days = 30 ): array {
		...
		foreach ( (array) $rows as $r ) {
			...
			// Top keywords clicked at this specific rank position
			$top_keywords = (array) $wpdb->get_results( $wpdb->prepare( // phpcs:ignore
				"SELECT query, COUNT(*) as keyword_clicks, MAX(searched_at) as last_clicked
				 FROM {$table}
				 WHERE searched_at >= %s AND clicked_position = %d AND site_id = %d
				 GROUP BY query_hash
				 ORDER BY keyword_clicks DESC
				 LIMIT 5",
				$since, $pos, get_current_blog_id()
			), ARRAY_A );
	...
	public static function get_keyword_click_breakdown( int $limit = 25, int $days = 30 ): array {
		...
		return (array) $wpdb->get_results( $wpdb->prepare( // phpcs:ignore
			"SELECT query, clicked_position, COUNT(*) as clicks, MAX(searched_at) as last_clicked
			 FROM {$table}
			 WHERE searched_at >= %s AND clicked_position > 0 AND site_id = %d
