...
class Elasticsearch implements EngineInterface {
	...
	private function ensure_index(): bool {
		...
		$this->index_ready = ! is_wp_error( $create );
		return $this->index_ready;
	}
	...
	private function request( string $method, string $path, ?array $body = null ) {
		...
		if ( $code >= 400 ) {
			return new \WP_Error( 'http_error', 'Elasticsearch returned HTTP ' . $code, wp_remote_retrieve_body( $resp ) );
		}
