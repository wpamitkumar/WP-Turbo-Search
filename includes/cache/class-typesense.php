...
class Typesense implements EngineInterface {
	...
	public function upsert_bulk( array $documents ): bool {
		if ( empty( $documents ) || ! $this->ensure_collection() ) {
			return false;
		}
	...
	private function ensure_collection(): bool {
		...
		$this->collection_ready = ! is_wp_error( $create );
	...
	private function request( string $method, string $path, ?array $body = null ) {
		...
		if ( $code >= 400 ) {
			return new \WP_Error( 'http_error', 'Typesense returned HTTP ' . $code, wp_remote_retrieve_body( $resp ) );
		}
