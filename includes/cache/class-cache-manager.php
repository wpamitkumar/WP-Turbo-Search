...
class CacheManager {
	...
	public function upsert( array $document ): bool {
		return $this->engine->upsert( $document );
	}
	...
	public function get_indexed_count(): int {
		return $this->engine->get_indexed_count();
	}
	...
	private function make_key( string $query, array $filters, int $per_page, int $page ): string {
		...
		return self::KEY_PREFIX . md5( "{$clean_query}|{$filter_str}|{$per_page}|{$page}|" . get_current_blog_id() );
	}
