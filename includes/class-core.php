...
final class Core {
	...
	public function get_engine(): CacheManager {
		...
		$engine_hash = md5( implode( '|', [
			$settings['search_engine']        ?? 'mysql',
		...
		if ( null !== $manager && $manager_hash === $engine_hash ) {
			return $manager;
		}
		...
		$manager      = new CacheManager( $engine, $settings );
		$manager_hash = $engine_hash;
		return $manager;
	}
	...
	public function intercept_core_search( \WP_Query $query ): void {
		...
		try {
			...
			if ( ! empty( $results['hits'] ) ) {
				$post_ids = wp_list_pluck( $results['hits'], 'post_id' );
				if ( ! empty( $post_ids ) ) {
