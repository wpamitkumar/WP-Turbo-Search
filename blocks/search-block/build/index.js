...
( function ( blocks, element, blockEditor, components, i18n ) {
	...
	function Edit( props ) {
		...
		return el( Fragment, null,
			...
			el( InspectorControls, null,
				...
				/* Search Behaviour */
				el( PanelBody, { title: __( '🔎 Search Behaviour & Features', 'wp-turbo-search' ), initialOpen: true },
					el( TextControl, {
						label: __( 'Placeholder Text', 'wp-turbo-search' ), value: a.placeholder,
					...
					el( 'div', { className: 'wpts-block-post-types', style: { marginBottom: 16 } },
						el( 'label', { className: 'components-base-control__label', style: { display: 'block', marginBottom: 6, fontWeight: 600 } },
							__( 'Filter by Post Types (Multiple)', 'wp-turbo-search' )
						),
						( function () {
							...
							return el( 'div', { style: { display: 'flex', flexDirection: 'column', gap: 6, background: '#f8fafc', padding: '10px 12px', borderRadius: 8, border: '1px solid #e2e8f0' } },
								el( CheckboxControl, {
									label: __( '🌐 All Indexed Types (Default)', 'wp-turbo-search' ),
									checked: isAll,
					...
					el( SelectControl, {
						label: __( 'Results Layout', 'wp-turbo-search' ),
						value: a.layout || '',
						options: [
							{ label: __( '🌐 Default (Inherit Global Settings)', 'wp-turbo-search' ), value: '' },
							{ label: __( '📄 List View', 'wp-turbo-search' ),                        value: 'list' },
							{ label: __( '⊞ Grid Cards', 'wp-turbo-search' ),                       value: 'grid' },
							{ label: __( '🗂️ Compact Card', 'wp-turbo-search' ),                     value: 'card' },
						],
						onChange: function ( v ) { set( { layout: v } ); },
						help: __( 'Choose between List View, Grid Cards, or Compact layout.', 'wp-turbo-search' )
					} ),
					el( ToggleControl, {
						label: __( 'Show Voice Search Button', 'wp-turbo-search' ),
						help: __( 'Allows visitors to speak their query. Works with HTTPS connection only (or localhost).', 'wp-turbo-search' ),
						checked: a.showVoice,
						onChange: function ( v ) { set( { showVoice: v } ); },
					} ),
					el( ToggleControl, {
						label: __( 'Show Command + K Spotlight Hint', 'wp-turbo-search' ),
						checked: a.enableCommandK,
						onChange: function ( v ) { set( { enableCommandK: v } ); },
					} ),
					el( RangeControl, {
						label: __( 'Results per Page', 'wp-turbo-search' ), value: a.perPage,
						onChange: function ( v ) { set( { perPage: v } ); },
						min: 1, max: 20, step: 1,
					} ),
					el( RangeControl, {
						label: __( 'Min Characters to Search', 'wp-turbo-search' ), value: a.minChars,
						onChange: function ( v ) { set( { minChars: v } ); },
						min: 1, max: 5, step: 1,
						help: __( 'Search fires after this many characters are typed.', 'wp-turbo-search' ),
					} ),
					el( ToggleControl, {
						label: __( 'Open results in new tab', 'wp-turbo-search' ),
				...
				/* Performance */
				el( PanelBody, { title: __( '⚡ Performance', 'wp-turbo-search' ), initialOpen: false },
					el( RangeControl, {
						label: __( 'Debounce (ms)', 'wp-turbo-search' ), value: a.debounce,
						onChange: function ( v ) { set( { debounce: v } ); },
						min: 0, max: 1000, step: 50,
						help: __( 'Wait after user stops typing before searching (recommended: 150–300ms).', 'wp-turbo-search' ),
					} ),
					el( RangeControl, {
						label: __( 'Throttle (ms)', 'wp-turbo-search' ), value: a.throttle,
						onChange: function ( v ) { set( { throttle: v } ); },
						min: 0, max: 2000, step: 100,
						help: __( 'Max fire frequency while typing. 0 = disabled. Use throttle OR debounce, not both.', 'wp-turbo-search' ),
					} )
				),

				/* Results Display */
				el( PanelBody, { title: __( '📋 Results Display & Commerce', 'wp-turbo-search' ), initialOpen: true },
					el( ToggleControl, {
						label: __( 'Categorized Multi-Tabs in Dropdown', 'wp-turbo-search' ),
						checked: a.categoryTabs,
						onChange: function ( v ) { set( { categoryTabs: v } ); },
					} ),
					el( ToggleControl, {
						label: __( 'WooCommerce Instant 1-Click Add-to-Cart', 'wp-turbo-search' ),
						checked: a.quickAddToCart,
						onChange: function ( v ) { set( { quickAddToCart: v } ); },
					} ),
					el( ToggleControl, {
						label: __( 'Show post type badge', 'wp-turbo-search' ),
						checked: a.showPostType,
						onChange: function ( v ) { set( { showPostType: v } ); },
					} ),
					el( ToggleControl, {
						label: __( 'Show excerpt', 'wp-turbo-search' ),
						checked: a.showExcerpt,
						onChange: function ( v ) { set( { showExcerpt: v } ); },
					} ),
					el( RangeControl, {
						label: __( 'Dropdown max height (px)', 'wp-turbo-search' ), value: a.dropdownMaxHeight,
				...
				/* Styling */
				el( PanelBody, { title: __( '🎨 Styling', 'wp-turbo-search' ), initialOpen: false },
					el( SelectControl, {
						label: __( 'Theme Preset', 'wp-turbo-search' ), value: a.theme,
						options: [
							{ label: __( 'Light (default)', 'wp-turbo-search' ), value: 'light'   },
							{ label: __( 'Dark',            'wp-turbo-search' ), value: 'dark'    },
							{ label: __( 'Minimal',         'wp-turbo-search' ), value: 'minimal' },
							{ label: __( 'Glassmorphism',   'wp-turbo-search' ), value: 'glass'   },
							{ label: __( 'Custom',          'wp-turbo-search' ), value: 'custom'  },
						],
						onChange: applyTheme,
					} ),
					el( SelectControl, {
						label: __( 'Input Size', 'wp-turbo-search' ), value: a.inputSize,
						options: [
							{ label: __( 'Small (42px)',  'wp-turbo-search' ), value: 'small'  },
							{ label: __( 'Medium (52px)', 'wp-turbo-search' ), value: 'medium' },
							{ label: __( 'Large (62px)',  'wp-turbo-search' ), value: 'large'  },
						],
						onChange: function ( v ) { set( { inputSize: v } ); },
					} ),
					el( RangeControl, {
						label: __( 'Max Width (px)', 'wp-turbo-search' ), value: a.maxWidth,
						onChange: function ( v ) { set( { maxWidth: v } ); },
						min: 200, max: 1200, step: 10,
					} ),
					el( RangeControl, {
						label: __( 'Border Radius (px)', 'wp-turbo-search' ), value: a.borderRadius,
						onChange: function ( v ) { set( { borderRadius: v } ); },
						min: 0, max: 40, step: 1,
					} ),

					el( 'p', { style: { fontWeight: 600, marginTop: 12, marginBottom: 4 } }, __( 'Primary Color', 'wp-turbo-search' ) ),
					el( ColorPicker, { color: a.primaryColor, enableAlpha: false,
						onChange: function ( v ) { set( { primaryColor: colorVal( v ), theme: 'custom' } ); } } ),

					el( 'p', { style: { fontWeight: 600, marginTop: 12, marginBottom: 4 } }, __( 'Background Color', 'wp-turbo-search' ) ),
					el( ColorPicker, { color: a.backgroundColor, enableAlpha: false,
						onChange: function ( v ) { set( { backgroundColor: colorVal( v ), theme: 'custom' } ); } } ),

					el( 'p', { style: { fontWeight: 600, marginTop: 12, marginBottom: 4 } }, __( 'Text Color', 'wp-turbo-search' ) ),
					el( ColorPicker, { color: a.textColor, enableAlpha: false,
						onChange: function ( v ) { set( { textColor: colorVal( v ), theme: 'custom' } ); } } ),

					el( 'p', { style: { fontWeight: 600, marginTop: 12, marginBottom: 4 } }, __( 'Border Color', 'wp-turbo-search' ) ),
					el( ColorPicker, { color: a.borderColor, enableAlpha: false,
						onChange: function ( v ) { set( { borderColor: colorVal( v ), theme: 'custom' } ); } } ),

					el( 'p', { style: { fontWeight: 600, marginTop: 12, marginBottom: 4 } }, __( 'Highlight Color', 'wp-turbo-search' ) ),
	...
	registerBlockType( 'wpts/search-bar', {
		title:       __( 'Turbo Search Bar', 'wp-turbo-search' ),
		description: __( 'Instant search with debounce, throttle, and full styling controls.', 'wp-turbo-search' ),
		category:    'widgets',
		icon:        'search',
